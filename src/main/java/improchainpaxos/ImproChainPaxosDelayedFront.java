...
import improchainpaxos.ipc.ExecuteReadReply;
import improchainpaxos.ipc.SubmitReadRequest;
import improchainpaxos.timers.ReconnectTimer;
...
public class ImproChainPaxosDelayedFront extends FrontendProto {
    ...
    private static final Logger logger = LogManager.getLogger(ImproChainPaxosDelayedFront.class);
    ...
    //new ChainPaxosDelayedFront(configProps, i, this)
    public ImproChainPaxosDelayedFront(Properties props, short protoIndex, Application app) throws IOException {
    ...
    /**
     * 发送ChainPaxosDelayedProto的submitBatchRequest
     * **/
    protected void onPeerBatchMessage(PeerBatchMessage msg, Host from, short sProto, int channel) {
        sendRequest(new SubmitBatchRequest(msg.getBatch()), ImproChainPaxosDelayedProto.PROTOCOL_ID);
    }
    ...
    /**
     * 在btach时间间隔内开启处理sendNewReadBatch()或sendNewWriteBatch()
     * */
    ...
    private void sendNewReadBatch() {
        ...
        readDataBuffer = new ArrayList<>(LOCAL_BATCH_SIZE);
        sendRequest(new SubmitReadRequest(internalId, getProtoId()), ImproChainPaxosMixedProto.PROTOCOL_ID);
        lastReadBatchTime = System.currentTimeMillis();
    }
