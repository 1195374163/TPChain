...
import improchainpaxos.messages.*;
import improchainpaxos.timers.*;
import improchainpaxos.utils.AcceptedValue;
import improchainpaxos.utils.InstanceState;
import improchainpaxos.utils.Membership;
import improchainpaxos.utils.SeqN;
...
public class ImproChainPaxosDelayedProto extends GenericProtocol {

    private static final Logger logger = LogManager.getLogger(ImproChainPaxosDelayedProto.class);
    ...
    *构造函数
     */
    public ImproChainPaxosDelayedProto(Properties props, EventLoopGroup workerGroup) throws UnknownHostException {
     ...
     * 执行读取操作，因为已经ack了
     * */
    private void  execRead(InstanceState ins) {
    ...
    private void executeMembershipOp(InstanceState instance) {
        ...
        if (o.opType == MembershipOp.OpType.REMOVE) {
            ...
            if (state == State.ACTIVE) {
                ...
                sendRequest(new GetSnapshotRequest(target, instance.iN), ImproChainPaxosDelayedFront.PROTOCOL_ID_BASE);
            }
        }
    }
